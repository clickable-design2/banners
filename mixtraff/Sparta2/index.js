(function (lib, img, cjs, ss) {

var p; // shortcut to reference prototypes

// library properties:
lib.properties = {
	width: 600,
	height: 300,
	fps: 24,
	color: "#620006",
	opacity: 1.00,
	manifest: [
		{src:"images/back.jpg?1478529038698", id:"back"},
		{src:"images/dfsd.png?1478529038698", id:"dfsd"},
		{src:"images/for_03.png?1478529038698", id:"for_03"},
		{src:"images/for_06.png?1478529038698", id:"for_06"},
		{src:"images/for_11.png?1478529038698", id:"for_11"},
		{src:"images/for_14.png?1478529038698", id:"for_14"},
		{src:"images/Meteor.png?1478529038698", id:"Meteor"},
		{src:"images/Stone.png?1478529038698", id:"Stone"},
		{src:"images/w256h2561380453915MouseOptions256x25632.png?1478529038698", id:"w256h2561380453915MouseOptions256x25632"}
	]
};



lib.ssMetadata = [];


// symbols:



(lib.back = function() {
	this.initialize(img.back);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,600,300);


(lib.dfsd = function() {
	this.initialize(img.dfsd);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,31,29);


(lib.for_03 = function() {
	this.initialize(img.for_03);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,221,152);


(lib.for_06 = function() {
	this.initialize(img.for_06);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,220,151);


(lib.for_11 = function() {
	this.initialize(img.for_11);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,212,192);


(lib.for_14 = function() {
	this.initialize(img.for_14);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,88,94);


(lib.Meteor = function() {
	this.initialize(img.Meteor);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,144,224);


(lib.Stone = function() {
	this.initialize(img.Stone);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,100,100);


(lib.w256h2561380453915MouseOptions256x25632 = function() {
	this.initialize(img.w256h2561380453915MouseOptions256x25632);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,256,256);


(lib.Символ82 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AgdDEIAAlOIhBAAIAAg5IC9AAIAAA5IhBAAIAAFOg");
	this.shape.setTransform(156.8,24.4);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("");
	this.shape_1.setTransform(130.3,24.4);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AhPDDIAAg3QAaACALgGQAOgHAEgUIAAgEIhVksIA9AAIAiCGIARBaIAOhaIAdiGIBAAAIhCEiQgGAcgIASQgJAVgNAMQgMANgVAFQgPAEgVAAIgSgBg");
	this.shape_2.setTransform(102,24.5);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AAmDEIg6icIgTAjIAAB5Ig9AAIAAmHIA9AAIAACrIBPirIA9AAIhVCvIBVDYg");
	this.shape_3.setTransform(79.8,24.4);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AAtDEIgLhIIhJAAIgLBIIg4AAIA+mHIBYAAIA/GHgAAaBHIgcjEIgeDEIA6AAg");
	this.shape_4.setTransform(56.1,24.4);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgdDEIAAlOIhBAAIAAg5IC9AAIAAA5IhBAAIAAFOg");
	this.shape_5.setTransform(34.7,24.4);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AAtDEIgLhIIhJAAIgLBIIg4AAIA+mHIBYAAIA/GHgAAaBHIgcjEIgeDEIA6AAg");
	this.shape_6.setTransform(13.4,24.4);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,169,60);


(lib.Символ66 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("rgba(66,66,66,0.898)").s().p("AiyBUQhKgjAAgxQAAgwBKgjQBLgiBngBQBoABBLAiQBKAjAAAwQAAAxhKAjQhLAihoAAQhnAAhLgig");
	this.shape.setTransform(25.4,11.9);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,50.7,23.8);


(lib.Символ58 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FF0000").ss(3,1,1).p("Am7AkIIckaIFbDJIokEkg");
	this.shape.setTransform(44.5,24.8);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,91.9,52.5);


(lib.Символ56 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FF0000").ss(3,1,1).p("ApwAzIL5mOIHoEaIsEGdg");
	this.shape.setTransform(62.6,34.8);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,128.1,72.7);


(lib.Символ37 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AgVCWIAAguIArAAIAAAugAgQBTIgGhrIAAh9IAtAAIAAB9IgGBrg");
	this.shape.setTransform(251.6,18.7);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("Ag8CWIAAgqQAUABAJgFQAKgFADgQIAAgDIhBjlIAvAAIAZBmIANBFIALhEIAWhnIAxAAIgzDeQgEAWgGANQgHAQgKAJQgJAKgQAEQgMADgQAAIgNAAg");
	this.shape_1.setTransform(238.9,18.7);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AAaCWIAAiBIg0AAIAACBIgvAAIAAkrIAvAAIAACBIA0AAIAAiBIAwAAIAAErg");
	this.shape_2.setTransform(220.9,18.7);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AgdCUQgNgFgKgKQgJgJgFgOQgEgOAAgSIAAibQAAgSAEgOQAFgOAJgKQAKgJANgFQANgFAQAAQARAAANAFQANAFAKAJQAJAKAEAOQAGAOAAASIAACbQAAASgGAOQgEAOgJAJQgKAKgNAFQgNAFgRAAQgQAAgNgFgAgQhnQgHAIAAAPIAAChQAAAPAHAHQAFAHALAAQALAAAHgHQAGgHAAgPIAAihQAAgPgGgIQgHgGgLAAQgLAAgFAGg");
	this.shape_3.setTransform(203.3,18.7);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AhFCWIAAkrIBFAAQARAAANAFQAOAFAIAJQAJAKAEAOQAFANAAATIAAAnQAAASgFANQgEAMgJAKQgIAJgOAFQgNAFgRAAIgVAAIAABxgAgVgEIAVAAQALAAAFgGQAGgHABgQIAAgtQgBgPgGgHQgFgGgLAAIgVAAg");
	this.shape_4.setTransform(187.3,18.7);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgdCUQgOgFgIgKQgKgJgFgOQgEgOAAgSIAAibQAAgSAEgOQAFgOAKgKQAIgJAOgFQANgFAQAAQARAAANAFQAOAFAIAJQAKAKAEAOQAFAOABASIAACbQgBASgFAOQgEAOgKAJQgIAKgOAFQgNAFgRAAQgQAAgNgFgAgQhnQgHAIAAAPIAAChQAAAPAHAHQAGAHAKAAQALAAAHgHQAGgHAAgPIAAihQAAgPgGgIQgHgGgLAAQgKAAgGAGg");
	this.shape_5.setTransform(170.2,18.7);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AhHCWIAAkrICCAAIAAArIhTAAIAABNIAZAAQASAAAOAFQAMAFAJAJQAKAKAEALQAEAOAAASIAAAhQAAASgEANQgEAOgKAKQgJAJgMAFQgOAFgSAAgAgYBrIAZAAQAMAAAGgGQAGgHABgQIAAgmQgBgPgGgHQgGgGgMAAIgZAAg");
	this.shape_6.setTransform(152.9,18.7);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AgdCUQgNgFgJgKQgKgJgFgOQgEgOAAgSIAAibQAAgSAEgOQAFgOAKgKQAJgJANgFQANgFAQAAQARAAANAFQANAFAKAJQAJAKAEAOQAGAOAAASIAACbQAAASgGAOQgEAOgJAJQgKAKgNAFQgNAFgRAAQgQAAgNgFgAgQhnQgHAIAAAPIAAChQAAAPAHAHQAFAHALAAQALAAAHgHQAGgHAAgPIAAihQAAgPgGgIQgHgGgLAAQgLAAgFAGg");
	this.shape_7.setTransform(135.4,18.7);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AAgC3IAAjRIgUBIIglCJIgwAAIAAkqIAqAAIAAC8IARhCIAkh6IA0AAIAAEqgAgTiKQgKgDgFgGQgGgHgEgJQgDgIAAgLIAdAAQABALAFAEQAFAEAHAAQAKAAAGgEQAEgEABgLIAeAAQgBALgDAIQgDAJgGAHQgGAGgKADQgKADgMAAQgKAAgJgDg");
	this.shape_8.setTransform(111.1,15.3);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AgdCUQgOgFgJgKQgJgJgEgOQgGgOAAgSIAAibQAAgSAGgOQAEgOAJgKQAJgJAOgFQANgFAQAAQARAAANAFQANAFAJAJQAKAKAFAOQAEAOAAASIAACbQAAASgEAOQgFAOgKAJQgJAKgNAFQgNAFgRAAQgQAAgNgFgAgRhnQgGAIAAAPIAAChQAAAPAGAHQAHAHAKAAQAMAAAFgHQAHgHAAgPIAAihQAAgPgHgIQgFgGgMAAQgKAAgHAGg");
	this.shape_9.setTransform(93.4,18.7);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AhECWIAAkrIBEAAQARAAANAFQANAFAJAJQAJAKAFAOQADANAAATIAAAnQAAASgDANQgFAMgJAKQgJAJgNAFQgNAFgRAAIgVAAIAABxgAgVgEIAVAAQALAAAFgGQAHgHAAgQIAAgtQAAgPgHgHQgFgGgLAAIgVAAg");
	this.shape_10.setTransform(77.4,18.7);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("AgWCWIAAkAIgyAAIAAgrICQAAIAAArIgxAAIAAEAg");
	this.shape_11.setTransform(61.1,18.7);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("");
	this.shape_12.setTransform(45.2,18.7);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("AgdCUQgNgFgKgKQgJgJgEgOQgGgOAAgSIAAibQAAgSAGgOQAEgOAJgKQAKgJANgFQANgFAQAAQARAAANAFQANAFAKAJQAJAKAEAOQAGAOgBASIAACbQABASgGAOQgEAOgJAJQgKAKgNAFQgNAFgRAAQgQAAgNgFgAgRhnQgGAIAAAPIAAChQAAAPAGAHQAGAHALAAQAMAAAFgHQAHgHAAgPIAAihQAAgPgHgIQgFgGgMAAQgLAAgGAGg");
	this.shape_13.setTransform(28.4,18.7);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AAaCWIAAkAIgzAAIAAEAIgwAAIAAkrICTAAIAAErg");
	this.shape_14.setTransform(10.7,18.7);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,257.4,46);


(lib.Символ31 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("rgba(255,255,255,0.498)").s().p("AuJJwIAAzfIcSAAIAATfg");
	this.shape.setTransform(90.6,62.5);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,181.2,124.9);


(lib.Символ30 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("rgba(255,255,255,0.498)").s().p("AuJJ0IAAznIcSAAIAATng");
	this.shape.setTransform(90.6,62.9);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,181.2,125.7);


(lib.Символ29 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(1,1,1).p("AFqAAQAACWhrBpQhpBriWAAQiVAAhqhrQhqhpAAiWQAAiVBqhqQBqhqCVAAQCWAABpBqQBrBqAACVg");
	this.shape.setTransform(36.2,36.2);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("rgba(255,255,255,0.498)").s().p("Aj+EAQhqhrgBiVQABiUBqhqQBqhqCUgBQCVABBrBqQBqBqgBCUQABCVhqBrQhrBqiVgBQiUABhqhqg");
	this.shape_1.setTransform(36.2,36.2);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1,-1,74.4,74.4);


(lib.Символ23 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FF0000").s().p("Ai1AZIAEgFIB6AAIAAgwIBwAAIAAAwIB5AAIADAFIi1EGgAg3gwIAAgtIBwAAIAAAtgAg3huIAAgxIBwAAIAAAxgAg3ivIAAgtIBwAAIAAAtgAg3jtIAAgxIBwAAIAAAxg");
	this.shape.setTransform(18.2,28.8);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,36.4,57.5);


(lib.Символ20 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.for_14();
	this.instance.parent = this;
	this.instance.setTransform(3.9,1.1,0.709,0.709);

	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(1,1,1).p("AFqAAQAACWhrBpQhpBriWAAQiVAAhqhrQhqhpAAiWQAAiVBqhqQBqhqCVAAQCWAABpBqQBrBqAACVg");
	this.shape.setTransform(36.2,36.2);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("rgba(255,255,255,0.498)").s().p("Aj+EAQhqhrgBiVQABiUBqhqQBqhqCUgBQCVABBrBqQBqBqgBCUQABCVhqBrQhrBqiVgBQiUABhqhqg");
	this.shape_1.setTransform(36.2,36.2);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape},{t:this.instance}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1,-1,74.4,74.4);


(lib.Символ16 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AiSAAIDijiIAAB8IBDAAIAADNIhDAAIAAB8g");
	this.shape.setTransform(14.8,22.8);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,29.5,45.5);


(lib.Символ13 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.for_06();
	this.instance.parent = this;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 2
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(1,1,1).p("ApHpjISPAAIAATHIyPAAg");
	this.shape.setTransform(64,77.7);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.lf(["#620006","#D10303","#620006"],[0,0.502,1],-58.4,0,58.5,0).s().p("ApHJkIAAzHISPAAIAATHg");
	this.shape_1.setTransform(64,77.7);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,220,151);


(lib.Символ12 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.for_03();
	this.instance.parent = this;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 2
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(1,1,1).p("ApTpHISnAAIAASPIynAAg");
	this.shape.setTransform(74.6,76.9);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.lf(["#620006","#D10303","#620006"],[0,0.502,1],-59.6,0,59.7,0).s().p("ApTJIIAAyPISnAAIAASPg");
	this.shape_1.setTransform(74.6,76.9);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,221,152);


(lib.Символ11 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 2
	this.shape = new cjs.Shape();
	this.shape.graphics.f("rgba(0,0,0,0.2)").s().p("EgyuAbEMAAAg2GMBldAAAMAAAA2Gg");
	this.shape.setTransform(314.2,157.8);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	// Слой 1
	this.instance = new lib.back();
	this.instance.parent = this;
	this.instance.setTransform(0,0,1.05,1.05);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-10.5,-15.4,649.4,346.4);


(lib.Символ8 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FF0000").ss(2.8,1,1).p("AEQiNQCpArAnBIQApBFhxA7QhxA7jIAMQjFAMipgrQiogrgphIQgnhGBwg7QBwg7DIgLQDHgMCoArg");
	this.shape.setTransform(49.2,17.6);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("rgba(255,0,0,0.4)").s().p("AkPCOQiogrgphIQgnhGBwg7QBwg7DIgLQDHgMCoArQCpArAnBIQApBFhxA7QhxA7jIAMQgqADgpAAQiWAAiFgig");
	this.shape_1.setTransform(49.2,17.6);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.4,-1.4,101.1,38);


(lib.Символ4 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#00FF00").ss(2.8,1,1).p("AEsnkIG4AAIAAG4Im4AAgAjbnkIG3AAIAAG4Im3AAgAjbAtIG3AAIAAG4Im3AAgAEsAtIG4AAIAAG4Im4AAgArjnkIG4AAIAAG4Im4AAgArjAtIG4AAIAAG4Im4AAg");
	this.shape.setTransform(74,48.5);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("rgba(0,255,0,0.498)").s().p("AEsHlIAAm4IG3AAIAAG4gAjaHlIAAm4IG2AAIAAG4gArjHlIAAm4IG5AAIAAG4gAEsgrIAAm5IG3AAIAAG5gAjagrIAAm5IG2AAIAAG5gArjgrIAAm5IG5AAIAAG5g");
	this.shape_1.setTransform(74,48.5);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.4,-1.4,150.8,99.8);


(lib.Символ3 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.shape = new cjs.Shape();
	this.shape.graphics.lf(["#620006","#D10303","#620006"],[0,0.502,1],0,42.8,0,-42.8).s().p("EghRAGjICrtGMA/3AAAIAANGg");
	this.shape.setTransform(213,42);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,426,84);


(lib.Анимация2 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.dfsd();
	this.instance.parent = this;
	this.instance.setTransform(-15.5,-14.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-15.5,-14.5,31,29);


(lib.Анимация1 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.dfsd();
	this.instance.parent = this;
	this.instance.setTransform(-15.5,-14.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-15.5,-14.5,31,29);


(lib.Анимация8 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Meteor();
	this.instance.parent = this;
	this.instance.setTransform(27.9,-56,0.47,0.47,59.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-62.5,-56,125,112);


(lib.Анимация7 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Meteor();
	this.instance.parent = this;
	this.instance.setTransform(29,-58,0.487,0.487,59.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-64.8,-58,129.6,116.1);


(lib.Анимация6 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Stone();
	this.instance.parent = this;
	this.instance.setTransform(-19.9,-19.9,0.399,0.399);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-19.9,-19.9,39.9,39.9);


(lib.Анимация5 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Stone();
	this.instance.parent = this;
	this.instance.setTransform(-19.9,-19.9,0.399,0.399);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-19.9,-19.9,39.9,39.9);


(lib.Tween22 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.instance = new lib.w256h2561380453915MouseOptions256x25632();
	this.instance.parent = this;
	this.instance.setTransform(-30,-30,0.234,0.234);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-30,-30,60,60);


(lib.shape109 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(3,1,1).p("AETAAQAABxhRBRQhRBRhxAAQhwAAhRhRQhRhRAAhxQAAhwBRhRQBRhRBwAAQBxAABRBRQBRBRAABwg");
	this.shape.setTransform(27.5,27.5);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,58,58);


(lib.shape34 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("");
	this.shape.setTransform(-47.5,76.4);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E78A0E").s().p("");
	this.shape_1.setTransform(-31.2,56.2);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-72.3,20.2,82.2,72.1);


(lib.shape33 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("AB+BLQgXgKg9AAIhKAGIg5AFIgXgRQgegaAAgcIADgPIACgVIAVAAQAGAIANAZQAFgNAcgdIAngoQgGARAXAkQAXAmAgAAIAigIIAggFIAVACIABApQACAdAGALg");
	this.shape.setTransform(-52.5,82.8);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E78A0E").s().p("");
	this.shape_1.setTransform(-27.3,53.9);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-74.3,16.5,93.9,74.9);


(lib.shape32 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("");
	this.shape.setTransform(-29.4,78.1);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E18403").s().p("");
	this.shape_1.setTransform(-27.2,54.8);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-74.3,15.6,94.1,79.6);


(lib.shape31 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("");
	this.shape.setTransform(-16.5,49.9);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E78A0E").s().p("");
	this.shape_1.setTransform(-31.1,54.2);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-70.8,14.4,79.5,79.6);


(lib.shape30 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFBAA").s().p("AhLCFQgMgaAAgUQAAgnCvjOIgyBbQg1BkAAAMQAAANAGAOQAGANAAANQAAAGgGAYIgFAWIgBAFQgIAFgGABIgUABQgNAAgNgdgAhRhvQgGgKAAgRIADgOQAFgJAIAAIAIABIADAAQADADgCAPIAEAIQAGALAAAEIAAAKQgCAJgLAAQgMAAgHgLg");
	this.shape.setTransform(-24.5,49);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E5A90A").s().p("");
	this.shape_1.setTransform(-49.8,76.6);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFDF18").s().p("");
	this.shape_2.setTransform(-43.9,54.7);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#E18403").s().p("");
	this.shape_3.setTransform(-19.7,64.6);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-72.6,17.1,70.4,75.2);


(lib.shape29 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFBAA").s().p("AhbFIQgGgOAAgTQAAgSADgGQAMAAAQAPIAXgIIAigDQAJgBAOgQQAQgTAJgaIAIAMQABAFAAAPQAAAVgGAYQgFAWgHALIgFAAIhGAIgAAMkBIgYg1IgCgFQAAgNAIgCIALAAQANAAAlASIArAXQgZABgPAPQgOAOAAARQAAAPAGAdIAAADQgIgHgeg3g");
	this.shape.setTransform(-42.6,60.9);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFDF18").s().p("");
	this.shape_1.setTransform(-38.1,57.7);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#E18403").s().p("");
	this.shape_2.setTransform(-36,54.4);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-67.7,16.2,63.5,77.8);


(lib.shape28 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFBAA").s().p("");
	this.shape.setTransform(-40.1,62);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFDF18").s().p("");
	this.shape_1.setTransform(-38.2,59.8);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#E18403").s().p("");
	this.shape_2.setTransform(-40,57.7);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-72.3,24.3,64.6,67.2);


(lib.shape27 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("");
	this.shape.setTransform(-29.5,66.4);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E18403").s().p("");
	this.shape_1.setTransform(-33.5,57.3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-71.4,21.7,75.8,71.1);


(lib.shape26 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFBAA").s().p("AhgA2IgHABQAHgTASgVIAognQAZgXANgEIAzgFIAVACIAdABIABABIACACQg8ATgYAUQgdAXgFAtg");
	this.shape.setTransform(-47.7,85.7);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFDF18").s().p("");
	this.shape_1.setTransform(-27.5,67.4);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#E78A0E").s().p("");
	this.shape_2.setTransform(-28.2,62.5);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-72.3,31.5,88.2,62);


(lib.shape25 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFBAA").s().p("AgMASQgMgDAAgNQAAgHAHgGQAKgIASACQAWAKgNARQgKAIgKAAg");
	this.shape.setTransform(-50,87.3);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFDF18").s().p("");
	this.shape_1.setTransform(-31.1,69.2);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#E18403").s().p("");
	this.shape_2.setTransform(-11.4,65.3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-72.3,39.2,105.2,52.9);


(lib.shape24 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#E18403").s().p("");
	this.shape.setTransform(-13.2,67.4);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FBFD9B").s().p("");
	this.shape_1.setTransform(-32.6,74);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFDF18").s().p("");
	this.shape_2.setTransform(-33.7,70.2);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-73.5,42.3,86.8,50.9);


(lib.shape23 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("");
	this.shape.setTransform(-30.2,79.2);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E18403").s().p("");
	this.shape_1.setTransform(-28.9,70.1);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-70.5,45,83.3,50.3);


(lib.shape22 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#E18403").s().p("");
	this.shape.setTransform(-17.6,68.8);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFDF18").s().p("");
	this.shape_1.setTransform(-31.5,80.2);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-69.1,45,103.1,48.2);


(lib.shape21 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("ABHBDQgMgEAAgIQAAgIAPgXQAOgYAAgJQAAgQgHgFIgZgNQABAAABAAQAAAAAAgBQABAAAAgBQAAAAAAgBIAVgDIAYgDIAhAEIAdALQABApgRAdQgWAlgrAAgAiUAsQgRgSAAgdIABgeQAEgSARgGIAHAiQAHARATAAQAZAAArgbQAwggAQgEIAdAAIgoAgQgeAYgCANQgEASgYAWQgaAYgYAAQgfAAgSgUg");
	this.shape.setTransform(-47.9,81);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E78A0E").s().p("");
	this.shape_1.setTransform(-28.4,64.3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-73.5,35.4,90.3,57.8);


(lib.shape20 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFDF18").s().p("");
	this.shape.setTransform(-43.6,66.6);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E78A0E").s().p("");
	this.shape_1.setTransform(-28.1,63.7);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-72.3,33.6,88.5,60.1);


(lib.Символ81 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape34("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(72.4,-20.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,82.2,72.1);


(lib.Символ80 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape33("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(74.3,-16.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,93.9,74.9);


(lib.Символ79 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape32("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(74.3,-15.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,94.1,79.6);


(lib.Символ78 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape31("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(70.9,-14.4);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,79.5,79.6);


(lib.Символ77 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape30("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(72.7,-17);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,70.4,75.2);


(lib.Символ76 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape29("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(67.7,-16.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,63.5,77.8);


(lib.Символ75 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape28("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(72.4,-24.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,64.7,67.2);


(lib.Символ74 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape27("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(71.5,-21.7);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,75.8,71.2);


(lib.Символ73 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape26("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(72.4,-31.4);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,88.2,62);


(lib.Символ72 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape25("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(72.4,-39.1);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,105.2,52.9);


(lib.Символ71 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape24("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(73.5,-42.3);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,86.8,50.9);


(lib.Символ70 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape23("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(70.6,-44.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,83.3,50.3);


(lib.Символ69 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape22("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(69.2,-44.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,103.1,48.2);


(lib.Символ68 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape21("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(73.5,-35.4);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,90.3,57.8);


(lib.Символ67 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.shape20("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(72.4,-33.6);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,88.5,60.1);


(lib.Символ65 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Анимация6("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(4.1,4.1,0.205,0.205);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,8.2,8.2);


(lib.Символ64 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Анимация6("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(4.1,4.1,0.205,0.205);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,8.2,8.2);


(lib.Символ63 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Анимация6("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(4.1,4.1,0.205,0.205,0,0,0,-0.2,0);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,8.2,8.2);


(lib.Символ62 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Анимация6("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(4.1,4.1,0.205,0.205,0,0,0,-0.2,0);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,8.2,8.2);


(lib.Символ61 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Анимация6("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(4.1,4.1,0.205,0.205,0,0,0,-0.2,0);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,8.2,8.2);


(lib.Символ57 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ58();
	this.instance.parent = this;
	this.instance.setTransform(44.5,24.8,1,1,0,0,0,44.5,24.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,91.9,52.5);


(lib.Символ55 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ56();
	this.instance.parent = this;
	this.instance.setTransform(62.6,34.8,1,1,0,0,0,62.6,34.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({alpha:0},4).to({alpha:1},5).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,128.1,72.7);


(lib.Символ49 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ55();
	this.instance.parent = this;
	this.instance.setTransform(62.6,34.8,1,1,0,0,0,62.6,34.8);

	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FF0000").ss(3,1,1).p("ApwAzIL5mOIHoEaIsEGdg");
	this.shape.setTransform(62.6,34.8);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("rgba(255,0,0,0.4)").s().p("ApwAzIL5mOIHoEaIsEGdg");
	this.shape_1.setTransform(62.6,34.8);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance}]},1).to({state:[{t:this.shape_1},{t:this.shape}]},2).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = null;


(lib.Символ46 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ57();
	this.instance.parent = this;
	this.instance.setTransform(44.5,24.8,1,1,0,0,0,44.5,24.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({alpha:0},4).to({alpha:1},5).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,91.9,52.5);


(lib.Символ39 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ37();
	this.instance.parent = this;
	this.instance.setTransform(128.7,23,1,1,0,0,0,128.7,23);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,257.4,46);


(lib.Символ38 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 2
	this.instance = new lib.Символ82();
	this.instance.parent = this;
	this.instance.setTransform(129.4,23,0.214,0.214,0,0,0,84.5,30.2);
	this.instance.alpha = 0;
	this.instance._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(75).to({_off:false},0).to({regY:30.1,scaleX:0.78,scaleY:0.78,y:23.1,alpha:1},8,cjs.Ease.get(-1)).to({scaleX:0.82,x:133.4,y:23.2},9,cjs.Ease.get(-1)).to({scaleX:0.87,x:137.4,y:23.1},9,cjs.Ease.get(1)).to({regY:29.9,scaleX:0.82,x:133.1,y:23},10,cjs.Ease.get(-1)).to({regY:30.1,scaleX:0.78,x:129.4,y:23.1},9,cjs.Ease.get(1)).to({scaleX:0.83,x:125.3,y:23.2},9,cjs.Ease.get(-1)).to({scaleX:0.88,x:120.7,y:23.1},10,cjs.Ease.get(1)).to({regX:84.6,regY:29.9,scaleX:0.83,x:125.3,y:23},10,cjs.Ease.get(-1)).to({regX:84.5,regY:30.1,scaleX:0.78,x:129.4,y:23.1},9,cjs.Ease.get(1)).wait(1).to({regY:30.2,scaleX:0.21,scaleY:0.21,y:23,alpha:0},7,cjs.Ease.get(1)).wait(1));

	// Слой 1
	this.instance_1 = new lib.Символ39();
	this.instance_1.parent = this;
	this.instance_1.setTransform(128.7,23,1,1,0,0,0,128.7,23);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).to({scaleX:1.02,x:125.5},9,cjs.Ease.get(-1)).to({scaleX:1.1,x:115.9},9,cjs.Ease.get(0.99)).to({scaleX:1.07,x:119.5},10,cjs.Ease.get(-1)).to({scaleX:1,x:128.7},9,cjs.Ease.get(0.99)).to({scaleX:1.02,x:131.8},9,cjs.Ease.get(-1)).to({scaleX:1.11,x:142.3},10,cjs.Ease.get(0.99)).to({scaleX:1.08,x:138.6},10,cjs.Ease.get(-1)).to({scaleX:1,x:128.7},9,cjs.Ease.get(0.99)).to({scaleX:0.33,scaleY:0.33,alpha:0},8).to({_off:true},1).wait(74).to({_off:false},0).to({scaleX:1,scaleY:1,alpha:1},8).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,257.4,46);


(lib.Символ36 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ31();
	this.instance.parent = this;
	this.instance.setTransform(90.6,62.5,1,1,0,0,0,90.6,62.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,181.2,124.9);


(lib.Символ35 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ36();
	this.instance.parent = this;
	this.instance.setTransform(90.6,62.5,1,1,0,0,0,90.6,62.5);
	this.instance._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(3).to({_off:false},0).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = null;


(lib.Символ32 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ30();
	this.instance.parent = this;
	this.instance.setTransform(90.6,62.9,1,1,0,0,0,90.6,62.9);
	this.instance._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(3).to({_off:false},0).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = null;


(lib.Символ28 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 2
	this.instance = new lib.for_11();
	this.instance.parent = this;
	this.instance.setTransform(2,2,0.321,0.321);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 1
	this.instance_1 = new lib.Символ29();
	this.instance_1.parent = this;
	this.instance_1.setTransform(36.2,36.2,1,1,0,0,0,36.2,36.2);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.5,-0.5,73.4,73.4);


(lib.Символ27 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ28();
	this.instance.parent = this;
	this.instance.setTransform(36.2,36.2,1,1,0,0,0,36.2,36.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({scaleX:1.02,scaleY:1.02},4,cjs.Ease.get(-1)).to({regX:36.1,regY:36.1,scaleX:1.06,scaleY:1.06},5,cjs.Ease.get(1)).to({scaleX:1.03,scaleY:1.03},5,cjs.Ease.get(-1)).to({regX:36.2,regY:36.2,scaleX:1,scaleY:1},5,cjs.Ease.get(1)).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.5,-0.5,73.4,73.4);


(lib.Символ25 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ23();
	this.instance.parent = this;
	this.instance.setTransform(18.2,28.8,1,1,0,0,0,18.2,28.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,36.4,57.5);


(lib.Символ24 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 3 (mask)
	var mask = new cjs.Shape();
	mask._off = true;
	mask.graphics.p("AkWNiIAA8kQEnBYEGhYIAAckQiVBhiJAAQiMAAiDhhg");
	mask.setTransform(17.2,80.4);

	// Слой 1 - копия: 2
	this.instance = new lib.Символ25();
	this.instance.parent = this;
	this.instance.setTransform(18.2,61.1,1,1,0,0,0,18.2,28.8);

	this.instance.mask = mask;

	this.timeline.addTween(cjs.Tween.get(this.instance).to({y:221.9},35).wait(1).to({y:-49.2},0).to({y:56.5},23).wait(1));

	// Слой 1 - копия
	this.instance_1 = new lib.Символ25();
	this.instance_1.parent = this;
	this.instance_1.setTransform(18.2,148.4,1,1,0,0,0,18.2,28.8);

	this.instance_1.mask = mask;

	this.timeline.addTween(cjs.Tween.get(this.instance_1).to({y:221.9},16).wait(1).to({y:-49.2},0).to({y:143.8},42).wait(1));

	// Слой 1
	this.instance_2 = new lib.Символ25();
	this.instance_2.parent = this;
	this.instance_2.setTransform(18.2,-49.2,1,1,0,0,0,18.2,28.8);

	this.instance_2.mask = mask;

	this.timeline.addTween(cjs.Tween.get(this.instance_2).to({y:221.9},59).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,32.3,36.4,144.5);


(lib.Символ22 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ20();
	this.instance.parent = this;
	this.instance.setTransform(36.2,36.2,1,1,0,0,0,36.2,36.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.5,-0.5,73.4,73.4);


(lib.Символ21 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ22();
	this.instance.parent = this;
	this.instance.setTransform(36.2,36.2,1,1,0,0,0,36.2,36.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({scaleX:1.07,scaleY:1.07},4,cjs.Ease.get(-1)).to({scaleX:1.17,scaleY:1.17,x:36.3,y:36.3},5,cjs.Ease.get(1)).to({regX:36.1,regY:36.1,scaleX:1.08,scaleY:1.08,x:36.2,y:36.2},5,cjs.Ease.get(-1)).to({regX:36.2,regY:36.2,scaleX:1,scaleY:1},5,cjs.Ease.get(1)).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.5,-0.5,73.4,73.4);


(lib.Символ14 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ16();
	this.instance.parent = this;
	this.instance.setTransform(14.8,22.8,1,1,0,0,0,14.8,22.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,29.5,45.5);


(lib.Символ9 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ8();
	this.instance.parent = this;
	this.instance.setTransform(73,27.8,1,1,0,0,0,49.1,17.6);

	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FF0000").ss(2.8,1,1).p("ACCgFIJEAAAATkaIAADbAAWBAIAADbArFgFIJsAA");
	this.shape.setTransform(71,28.3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape},{t:this.instance}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.4,-1.4,144.8,59.4);


(lib.Символ7 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Символ 4
	this.instance = new lib.Символ4();
	this.instance.parent = this;
	this.instance.setTransform(127.6,70.7,0.378,0.351,0,-59.7,-28,74.2,48.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({alpha:0.172},4).to({alpha:1},5).wait(13));

	// Символ 4
	this.instance_1 = new lib.Символ4();
	this.instance_1.parent = this;
	this.instance_1.setTransform(39.6,79.1,0.378,0.351,0,-59.7,-28,74.2,48.9);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(4).to({alpha:0.172},4).to({alpha:1},5).wait(9));

	// Символ 4
	this.instance_2 = new lib.Символ4();
	this.instance_2.parent = this;
	this.instance_2.setTransform(93.6,50.7,0.378,0.351,0,-59.7,-28,74.2,48.9);

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(8).to({alpha:0.172},4).to({alpha:1},5).wait(5));

	// Символ 4
	this.instance_3 = new lib.Символ4();
	this.instance_3.parent = this;
	this.instance_3.setTransform(146.6,21.8,0.378,0.351,0,-59.7,-28,74.2,48.9);

	this.timeline.addTween(cjs.Tween.get(this.instance_3).wait(12).to({alpha:0.172},4).to({alpha:1},5).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.8,-0.5,187.5,101.8);


(lib.Символ6 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Символ 4
	this.instance = new lib.Символ4();
	this.instance.parent = this;
	this.instance.setTransform(170.6,31.3,0.545,0.506,0,-59.7,-27.9,74.2,48.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({alpha:0.172},4).to({alpha:1},5).wait(5));

	// Символ 4
	this.instance_1 = new lib.Символ4();
	this.instance_1.parent = this;
	this.instance_1.setTransform(57,86.3,0.545,0.506,0,-59.7,-27.9,74.2,48.8);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(4).to({alpha:0.172},4).to({alpha:1},5).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.2,-0.7,229.8,119);


(lib.Символ5 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ7();
	this.instance.parent = this;
	this.instance.setTransform(92.9,50.4,1,1,0,0,0,92.9,50.4);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.8,-0.5,187.5,101.8);


(lib.Символ2 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// построй оборону!
	this.instance = new lib.Символ38();
	this.instance.parent = this;
	this.instance.setTransform(172.9,141,1,1,0,0,0,128.7,23);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 1
	this.instance_1 = new lib.Символ3();
	this.instance_1.parent = this;
	this.instance_1.setTransform(213,128,1,1,0,0,0,213,42);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,86,426,84);


(lib.Символ11_1 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 4
	this.instance_1 = new lib.Анимация1("synched",0);
	this.instance_1.parent = this;
	this.instance_1.setTransform(15.5,1.3,0.685,0.685);

	this.instance_2 = new lib.Анимация2("synched",0);
	this.instance_2.parent = this;
	this.instance_2.setTransform(15.5,-37.5,1.2,1.2);
	this.instance_2.alpha = 0;

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_1}]}).to({state:[{t:this.instance_1}]},1).to({state:[{t:this.instance_2}]},110).to({state:[{t:this.instance_1}]},1).to({state:[{t:this.instance_1}]},37).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_1).to({scaleX:0.7,scaleY:0.7,y:0.9},1).to({_off:true,scaleX:1.2,scaleY:1.2,y:-37.5,alpha:0},110).wait(1).to({_off:false,scaleX:0.23,scaleY:0.23,y:14.5,alpha:0.41},0).to({scaleX:0.67,scaleY:0.67,y:1.6,alpha:0.969},37).wait(1));

	// Слой 3
	this.instance_3 = new lib.Анимация1("synched",0);
	this.instance_3.parent = this;
	this.instance_3.setTransform(15.5,-12,0.866,0.866);
	this.instance_3.alpha = 0.66;

	this.instance_4 = new lib.Анимация2("synched",0);
	this.instance_4.parent = this;
	this.instance_4.setTransform(15.5,-37.5,1.2,1.2);
	this.instance_4.alpha = 0;

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_3}]}).to({state:[{t:this.instance_4}]},73).to({state:[{t:this.instance_3}]},1).to({state:[{t:this.instance_3}]},39).to({state:[{t:this.instance_3}]},36).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_3).to({_off:true,scaleX:1.2,scaleY:1.2,y:-37.5,alpha:0},73).wait(1).to({_off:false,scaleX:0.23,scaleY:0.23,y:14.5,alpha:0.41},0).to({scaleX:0.7,scaleY:0.7,y:0.9,alpha:1},39).to({scaleX:0.86,scaleY:0.86,y:-11.6,alpha:0.672},36).wait(1));

	// Слой 2
	this.instance_5 = new lib.Анимация1("synched",0);
	this.instance_5.parent = this;
	this.instance_5.setTransform(15.5,-24.6,1.031,1.031);
	this.instance_5.alpha = 0.34;

	this.instance_6 = new lib.Анимация2("synched",0);
	this.instance_6.parent = this;
	this.instance_6.setTransform(15.5,-37.5,1.2,1.2);
	this.instance_6.alpha = 0;

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_5}]}).to({state:[{t:this.instance_6}]},37).to({state:[{t:this.instance_5}]},1).to({state:[{t:this.instance_5}]},39).to({state:[{t:this.instance_5}]},72).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_5).to({_off:true,scaleX:1.2,scaleY:1.2,y:-37.5,alpha:0},37).wait(1).to({_off:false,scaleX:0.23,scaleY:0.23,y:14.5,alpha:0.41},0).to({scaleX:0.7,scaleY:0.7,y:0.9,alpha:1},39).to({scaleX:1.03,scaleY:1.03,y:-24.3,alpha:0.34},72).wait(1));

	// Слой 1
	this.instance_7 = new lib.Анимация1("synched",0);
	this.instance_7.parent = this;
	this.instance_7.setTransform(15.5,14.5,0.232,0.232);
	this.instance_7.alpha = 0.41;

	this.instance_8 = new lib.Анимация2("synched",0);
	this.instance_8.parent = this;
	this.instance_8.setTransform(15.5,-37.5,1.2,1.2);
	this.instance_8.alpha = 0;

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_7}]}).to({state:[{t:this.instance_7}]},39).to({state:[{t:this.instance_8}]},110).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_7).to({scaleX:0.7,scaleY:0.7,y:0.9,alpha:1},39).to({_off:true,scaleX:1.2,scaleY:1.2,y:-37.5,alpha:0},110).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-0.4,-39.5,32,57.4);


(lib.sprite110 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.instance = new lib.shape109("synched",0);
	this.instance.parent = this;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.5,-1.5,58,58);


(lib.sprite35 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.instance = new lib.Символ67();
	this.instance.parent = this;
	this.instance.setTransform(0.8,-8.3,1,1,0,0,0,44.2,13.2);
	this.instance.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance.cache(-2,-2,93,64);

	this.instance_1 = new lib.Символ68();
	this.instance_1.parent = this;
	this.instance_1.setTransform(0.5,-8.5,1,1,0,0,0,45.1,11.2);
	this.instance_1.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_1.cache(-2,-2,94,62);

	this.instance_2 = new lib.Символ69();
	this.instance_2.parent = this;
	this.instance_2.setTransform(11.3,-8.6,1,1,0,0,0,51.5,1.6);
	this.instance_2.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_2.cache(-2,-2,107,52);

	this.instance_3 = new lib.Символ70();
	this.instance_3.parent = this;
	this.instance_3.setTransform(0,-7.6,1,1,0,0,0,41.6,2.6);
	this.instance_3.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_3.cache(-2,-2,87,54);

	this.instance_4 = new lib.Символ71();
	this.instance_4.parent = this;
	this.instance_4.setTransform(-1.2,-8.6,1,1,0,0,0,43.4,4.2);
	this.instance_4.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_4.cache(-2,-2,91,55);

	this.instance_5 = new lib.Символ72();
	this.instance_5.parent = this;
	this.instance_5.setTransform(9.2,-9.1,1,1,0,0,0,52.6,6.9);
	this.instance_5.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_5.cache(-2,-2,109,57);

	this.instance_6 = new lib.Символ73();
	this.instance_6.parent = this;
	this.instance_6.setTransform(0.7,-8.5,1,1,0,0,0,44.1,15.2);
	this.instance_6.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_6.cache(-2,-2,92,66);

	this.instance_7 = new lib.Символ74();
	this.instance_7.parent = this;
	this.instance_7.setTransform(-4.6,-8.7,1,1,0,0,0,37.9,24.7);
	this.instance_7.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_7.cache(-2,-2,80,75);

	this.instance_8 = new lib.Символ75();
	this.instance_8.parent = this;
	this.instance_8.setTransform(-7.3,-9.5,1,1,0,0,0,36.1,21.4);
	this.instance_8.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_8.cache(-2,-2,69,71);

	this.instance_9 = new lib.Символ76();
	this.instance_9.parent = this;
	this.instance_9.setTransform(-4.9,-8.1,1,1,0,0,0,33.9,30.8);
	this.instance_9.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_9.cache(-2,-2,68,82);

	this.instance_10 = new lib.Символ77();
	this.instance_10.parent = this;
	this.instance_10.setTransform(-7.4,-9,1,1,0,0,0,36.3,29.1);
	this.instance_10.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_10.cache(-2,-2,74,79);

	this.instance_11 = new lib.Символ78();
	this.instance_11.parent = this;
	this.instance_11.setTransform(-2.1,-8.1,1,1,0,0,0,39.8,32.6);
	this.instance_11.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_11.cache(-2,-2,84,84);

	this.instance_12 = new lib.Символ79();
	this.instance_12.parent = this;
	this.instance_12.setTransform(1.7,-7.6,1,1,0,0,0,47.1,32);
	this.instance_12.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_12.cache(-2,-2,98,84);

	this.instance_13 = new lib.Символ80();
	this.instance_13.parent = this;
	this.instance_13.setTransform(1.6,-9.4,1,1,0,0,0,47,29.2);
	this.instance_13.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_13.cache(-2,-2,98,79);

	this.instance_14 = new lib.Символ81();
	this.instance_14.parent = this;
	this.instance_14.setTransform(-2.3,-9,1,1,0,0,0,41.1,25.9);
	this.instance_14.filters = [new cjs.BlurFilter(6, 6, 1)];
	this.instance_14.cache(-2,-2,86,76);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance}]}).to({state:[{t:this.instance_1}]},1).to({state:[{t:this.instance_2}]},1).to({state:[{t:this.instance_3}]},2).to({state:[{t:this.instance_4}]},1).to({state:[{t:this.instance_5}]},2).to({state:[{t:this.instance_6}]},2).to({state:[{t:this.instance_7}]},1).to({state:[{t:this.instance_8}]},2).to({state:[{t:this.instance_9}]},2).to({state:[{t:this.instance_10}]},1).to({state:[{t:this.instance_11}]},2).to({state:[{t:this.instance_12}]},1).to({state:[{t:this.instance_13}]},2).to({state:[{t:this.instance_14}]},1).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-46.4,-24.5,98,69);


(lib.Symbol16 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.instance = new lib.Tween22("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(30,30);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,60,60);


(lib.Символ60 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Анимация5("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(81,-15);

	this.instance_1 = new lib.Анимация6("synched",0);
	this.instance_1.parent = this;
	this.instance_1.setTransform(-325,-13);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance}]}).to({state:[{t:this.instance_1,p:{x:-325,y:-13}}]},10).to({state:[]},1).to({state:[{t:this.instance}]},49).to({state:[{t:this.instance_1,p:{x:-435,y:10.5}}]},10).to({state:[]},1).wait(49));
	this.timeline.addTween(cjs.Tween.get(this.instance).to({_off:true,rotation:360,x:-325,y:-13},10).wait(50).to({_off:false,x:81,y:-15},0).to({_off:true,rotation:720,x:-435,y:10.5},10).wait(50));

	// Анимация 6
	this.instance_2 = new lib.Символ61();
	this.instance_2.parent = this;
	this.instance_2.setTransform(-318.1,-5.5,1,1,0,0,0,4.1,4.1);
	this.instance_2._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(10).to({_off:false},0).to({rotation:-1440,x:-252.5,y:-17,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).to({_off:true},1).wait(10).to({_off:false,x:-422.8,y:16.6,alpha:1},0).to({rotation:-2880,x:-252.5,y:-17,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).wait(1));

	// Анимация 6
	this.instance_3 = new lib.Символ62();
	this.instance_3.parent = this;
	this.instance_3.setTransform(-335.1,-17.1,1,1,0,0,0,4.1,4.1);
	this.instance_3._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_3).wait(10).to({_off:false},0).to({rotation:-1440,x:-419.3,y:-61.8,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).to({_off:true},1).wait(10).to({_off:false,x:-439.8,y:5,alpha:1},0).to({rotation:-2880,x:-419.3,y:-61.8,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).wait(1));

	// Анимация 6
	this.instance_4 = new lib.Символ63();
	this.instance_4.parent = this;
	this.instance_4.setTransform(-334.4,-6.3,1,1,0,0,0,4.1,4.1);
	this.instance_4._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_4).wait(10).to({_off:false},0).to({rotation:-1440,x:-393.5,y:4.1,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).to({_off:true},1).wait(10).to({_off:false,x:-439,y:15.8,alpha:1},0).to({rotation:-2880,x:-393.5,y:4.1,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).wait(1));

	// Анимация 6
	this.instance_5 = new lib.Символ64();
	this.instance_5.parent = this;
	this.instance_5.setTransform(-317.7,-17.4,1,1,0,0,0,4.1,4.1);
	this.instance_5._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_5).wait(10).to({_off:false},0).to({rotation:-1440,x:-281.7,y:-89.4,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).to({_off:true},1).wait(10).to({_off:false,x:-422.4,y:4.7,alpha:1},0).to({rotation:-2880,x:-281.7,y:-89.4,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).wait(1));

	// Анимация 6
	this.instance_6 = new lib.Символ65();
	this.instance_6.parent = this;
	this.instance_6.setTransform(-327,-12.3,1,1,0,0,0,4.1,4.1);
	this.instance_6._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_6).wait(10).to({_off:false},0).to({rotation:-1440,x:-341.7,y:19.8,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).to({_off:true},1).wait(10).to({_off:false,x:-431.6,y:9.8,alpha:1},0).to({rotation:-2880,x:-341.7,y:19.8,alpha:0},10).to({_off:true},34).wait(5).to({_off:false},0).wait(1));

	// Слой 7
	this.instance_7 = new lib.Символ66();
	this.instance_7.parent = this;
	this.instance_7.setTransform(-323.4,-5.9,1,1,0,0,0,25.4,11.9);
	this.instance_7._off = true;
	this.instance_7.filters = [new cjs.BlurFilter(7, 7, 1)];
	this.instance_7.cache(-2,-2,55,28);

	this.timeline.addTween(cjs.Tween.get(this.instance_7).wait(10).to({_off:false},0).to({alpha:0},23).wait(37).to({x:-433.2,y:21.4,alpha:1},0).to({alpha:0},23).to({_off:true},1).wait(26));

	// Слой 9
	this.instance_8 = new lib.Анимация7("synched",0);
	this.instance_8.parent = this;
	this.instance_8.setTransform(70,-117.4,0.525,0.525,0,0,0,0,-0.1);
	this.instance_8._off = true;

	this.instance_9 = new lib.Анимация8("synched",0);
	this.instance_9.parent = this;
	this.instance_9.setTransform(-355.5,-103.8,0.476,0.476);
	this.instance_9._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_8).wait(47).to({_off:false},0).to({_off:true,regY:0,scaleX:0.48,scaleY:0.48,x:-355.5,y:-103.8},10).wait(63));
	this.timeline.addTween(cjs.Tween.get(this.instance_9).wait(47).to({_off:false},10).to({alpha:0},3).wait(60));

	// Слой 8
	this.instance_10 = new lib.sprite35();
	this.instance_10.parent = this;
	this.instance_10.setTransform(-388.8,-117.6,1.089,0.656,0,-1.9,178.1,0.1,-0.1);
	this.instance_10.compositeOperation = "lighter";

	this.timeline.addTween(cjs.Tween.get(this.instance_10).wait(28).to({regX:0,regY:-0.3,scaleX:0.29,scaleY:0.17,alpha:0},6).wait(24).to({regX:0.1,regY:-0.1,scaleX:1.09,scaleY:0.66,alpha:1},6).wait(56));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-441.2,-136.2,542.1,141.2);


(lib.Символ53 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ46();
	this.instance.parent = this;
	this.instance.setTransform(44.5,24.8,1,1,0,0,0,44.5,24.8);

	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FF0000").ss(3,1,1).p("Am7AkIIckaIFbDJIokEkg");
	this.shape.setTransform(44.5,24.8);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("rgba(255,0,0,0.4)").s().p("Am7AkIIdkaIFaDJIokEkg");
	this.shape_1.setTransform(44.5,24.8);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance}]},1).to({state:[{t:this.shape_1},{t:this.shape}]},2).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = null;


(lib.Символ41 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 3
	this.instance = new lib.Символ60();
	this.instance.parent = this;
	this.instance.setTransform(812.9,265.9,1,1,0,0,0,19.9,19.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 2
	this.instance_1 = new lib.Символ11_1();
	this.instance_1.parent = this;
	this.instance_1.setTransform(328.1,197.8,3.276,3.276,0,0,0,15.5,14.5);

	this.instance_2 = new lib.Символ11_1();
	this.instance_2.parent = this;
	this.instance_2.setTransform(328.1,197.8,3.276,3.276,0,0,0,15.5,14.5);

	this.instance_3 = new lib.Символ11_1();
	this.instance_3.parent = this;
	this.instance_3.setTransform(328.1,207.8,3.276,3.276,0,0,0,15.5,14.5);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_3},{t:this.instance_2},{t:this.instance_1}]}).wait(1));

	// Слой 1
	this.instance_4 = new lib.Символ11();
	this.instance_4.parent = this;
	this.instance_4.setTransform(333.5,172.9,1,1,0,0,0,315,157.5);

	this.timeline.addTween(cjs.Tween.get(this.instance_4).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(8,0,885.9,346.4);


(lib.Символ26 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 2
	this.instance = new lib.Символ27();
	this.instance.parent = this;
	this.instance.setTransform(64.2,-97,1,1,0,0,0,36.2,36.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Символ 24
	this.instance_1 = new lib.Символ24();
	this.instance_1.parent = this;
	this.instance_1.setTransform(65.6,-41.7,0.462,0.462,0,0,0,18.2,28.7);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(1));

	// Слой 1
	this.instance_2 = new lib.Символ9();
	this.instance_2.parent = this;
	this.instance_2.setTransform(63.8,25.5,0.899,0.899,0,0,0,71,28.4);

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.2,-133.7,130.1,185.8);


(lib.Символ19 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Символ 20
	this.instance = new lib.Символ21();
	this.instance.parent = this;
	this.instance.setTransform(64.4,-93.4,1,1,0,0,0,36.2,36.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 3
	this.instance_1 = new lib.Символ24();
	this.instance_1.parent = this;
	this.instance_1.setTransform(65.7,-41.1,0.462,0.462,0,0,0,18.2,28.7);

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(1));

	// Слой 1
	this.instance_2 = new lib.Символ9();
	this.instance_2.parent = this;
	this.instance_2.setTransform(63.8,25.5,0.899,0.899,0,0,0,71,28.4);

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.2,-130.1,130.1,182.3);


(lib.Символ17 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ9();
	this.instance.parent = this;
	this.instance.setTransform(72.1,37,0.899,0.899,0,0,0,71,28.4);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({guide:{path:[72.1,37.2,70.6,36.4,68.6,35.2,54.1,26.1,39.6,17.1,34.9,14.2,32.6,13,28.6,10.8,25.2,9.6,20.5,8,11.6,6.7,-3.8,4.4,-12.4,3.4,-25.8,1.7,-36.6,1.2,-43.3,1,-48.2,1.2,-54.4,1.5,-59.6,2.8,-65.4,4.2,-70.3,6.9,-75.6,9.9,-79.2,14.2,-83.6,19.6,-86.1,27.9,-89.1,37.6,-88.1,46,-86.8,57.1,-78.5,66.5,-70.8,75.3,-59.6,80.2,-49.9,84.4,-37.2,86,-27.7,87.1,-13.8,87,0.2,87,10.4,85.6,18.1,84.5,27.6,82.1,33.5,80.6,44.6,77.4,49.6,75.9,51.3,75.3,55,74,57.7,72.6,67.7,67.4,80.2,51,87.8,41.2,90.1,38.5,96,31.6,101.6,27.5,106.5,24,113.6,21,117.7,19.2,126.6,16.2,138.6,12.1,144.9,10.1,155.1,7,163.5,5.3,170.7,3.8,175.9,3.7,182.6,3.7,187.8,6,191.3,7.6,195,10.7,197.3,12.8,201.1,16.9,208.8,25.1,212.7,31,218.2,39.4,219.3,47.3,220.4,55.1,217.5,62.7,214.5,70.4,208.5,75.4,198.3,83.9,179.7,85,158.7,86.3,138.3,79.4,117.9,72.5,102,58.8,99.4,56.6,99.7,54.8,98.5,54.6,97.3,54.3]}},89).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(7.1,10.3,130.1,53.4);


(lib.Символ15 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ14();
	this.instance.parent = this;
	this.instance.setTransform(12.7,19.6,0.859,0.859,0,0,0,14.8,22.8);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({x:-19.3},4).to({x:12.7},5).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,25.4,39.1);


(lib.Символ10 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_1 = function() {
		this.stop();
	}
	this.frame_2 = function() {
		this.stop();
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(1).call(this.frame_1).wait(1).call(this.frame_2).wait(1));

	// Слой 1
	this.instance = new lib.Символ9();
	this.instance.parent = this;
	this.instance.setTransform(63.8,25.5,0.899,0.899,0,0,0,71,28.4);

	this.instance_1 = new lib.Символ19();
	this.instance_1.parent = this;
	this.instance_1.setTransform(63.8,25.4,1,1,0,0,0,63.8,25.4);

	this.instance_2 = new lib.Символ26();
	this.instance_2.parent = this;
	this.instance_2.setTransform(63.8,25.4,1,1,0,0,0,63.8,25.4);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance}]}).to({state:[{t:this.instance_1}]},1).to({state:[{t:this.instance_2}]},1).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-1.2,-1.2,130.1,53.4);


(lib.Анимация4 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ15();
	this.instance.parent = this;
	this.instance.setTransform(0,70.5,1,1,0,0,0,12.7,19.6);

	this.instance_1 = new lib.Символ15();
	this.instance_1.parent = this;
	this.instance_1.setTransform(0,-70.3,1,1,0,0,0,12.7,19.6);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_1},{t:this.instance}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-12.7,-89.9,25.4,179.9);


(lib.Анимация3 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ15();
	this.instance.parent = this;
	this.instance.setTransform(0,70.5,1,1,0,0,0,12.7,19.6);

	this.instance_1 = new lib.Символ15();
	this.instance_1.parent = this;
	this.instance_1.setTransform(0,-70.3,1,1,0,0,0,12.7,19.6);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_1},{t:this.instance}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-12.7,-89.9,25.4,179.9);


(lib.sprite112 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 1
	this.instance = new lib.Symbol16();
	this.instance.parent = this;
	this.instance.setTransform(1.5,21.8,0.85,0.85,0,0,0,30,30);

	this.timeline.addTween(cjs.Tween.get(this.instance).to({guide:{path:[1.5,21.7,1.5,21.3,1.5,20.9,1.5,15.3,5.6,11.2,9.7,7.1,15.3,7.1,21.1,7.1,25.2,11.2,29.3,15.3,29.3,20.9,29.3,26.7,25.2,30.8,21.1,34.9,15.3,34.9,9.7,34.9,5.6,30.8,3.3,28.5,2.3,25.7]}},29).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-24,-3.7,51,51);


(lib.Символ59 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 1
	this.instance = new lib.Символ41();
	this.instance.parent = this;
	this.instance.setTransform(316.7,173.2,1,1,0,0,0,324.7,173.2);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(10).to({x:310.2},0).wait(1).to({x:316.7,y:166.7},0).wait(1).to({y:179.7},0).wait(1).to({x:310.2,y:173.2},0).wait(1).to({x:303.7},0).wait(1).to({x:316.7},0).wait(55).to({x:310.2},0).wait(1).to({x:316.7,y:166.7},0).wait(1).to({y:179.7},0).wait(1).to({x:310.2,y:173.2},0).wait(1).to({x:303.7},0).wait(1).to({x:316.7},0).wait(45));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,885.9,346.4);


(lib.Символ11копия = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 2
	this.shape = new cjs.Shape();
	this.shape.graphics.f("rgba(0,0,0,0.2)").s().p("EgyuAbEMAAAg2GMBldAAAMAAAA2Gg");
	this.shape.setTransform(314.2,157.8);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	// Слой 1
	this.instance = new lib.Символ59();
	this.instance.parent = this;
	this.instance.setTransform(314.2,157.8,1,1,0,0,0,324.7,173.2);

	this.instance_1 = new lib.back();
	this.instance_1.parent = this;
	this.instance_1.setTransform(0,0,1.05,1.05);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_1},{t:this.instance}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-10.5,-15.4,885.9,346.4);


(lib.sprite113 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Layer 3
	this.instance = new lib.sprite112();
	this.instance.parent = this;
	this.instance.setTransform(11.6,6.4);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(30));

	// Layer 1
	this.instance_1 = new lib.sprite110();
	this.instance_1.parent = this;
	this.instance_1.setTransform(-8.7,-8.7,1.316,1.316);
	this.instance_1.alpha = 0.602;

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(30));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-12.5,-10.6,78.2,76.3);


(lib.Символ42 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// Слой 3
	this.instance = new lib.Символ53();
	this.instance.parent = this;
	this.instance.setTransform(554.3,288.5,1,1,0,0,0,44.5,24.8);
	new cjs.ButtonHelper(this.instance, 0, 1, 2, false, new lib.Символ53(), 3);

	this.instance_1 = new lib.Символ53();
	this.instance_1.parent = this;
	this.instance_1.setTransform(572.6,239,1,1,0,0,0,44.5,24.8);
	new cjs.ButtonHelper(this.instance_1, 0, 1, 2, false, new lib.Символ53(), 3);

	this.instance_2 = new lib.Символ53();
	this.instance_2.parent = this;
	this.instance_2.setTransform(519.3,268.3,1,1,0,0,0,44.5,24.8);
	new cjs.ButtonHelper(this.instance_2, 0, 1, 2, false, new lib.Символ53(), 3);

	this.instance_3 = new lib.Символ49();
	this.instance_3.parent = this;
	this.instance_3.setTransform(494.5,200.8,1,1,0,0,0,62.6,34.8);
	new cjs.ButtonHelper(this.instance_3, 0, 1, 2, false, new lib.Символ49(), 3);

	this.instance_4 = new lib.Символ53();
	this.instance_4.parent = this;
	this.instance_4.setTransform(465.4,297,1,1,0,0,0,44.5,24.8);
	new cjs.ButtonHelper(this.instance_4, 0, 1, 2, false, new lib.Символ53(), 3);

	this.instance_5 = new lib.Символ49();
	this.instance_5.parent = this;
	this.instance_5.setTransform(381.2,255.7,1,1,0,0,0,62.6,34.8);
	new cjs.ButtonHelper(this.instance_5, 0, 1, 2, false, new lib.Символ49(), 3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_5},{t:this.instance_4},{t:this.instance_3},{t:this.instance_2},{t:this.instance_1},{t:this.instance}]}).wait(1));

	// Слой 5
	this.kat = new lib.Символ6();
	this.kat.parent = this;
	this.kat.setTransform(437.1,228.1,1,1,0,0,0,113.6,58.8);

	this.timeline.addTween(cjs.Tween.get(this.kat).wait(1));

	// Символ 5
	this.sold = new lib.Символ5();
	this.sold.parent = this;
	this.sold.setTransform(518.9,268.1,1,1,0,0,0,92.9,50.4);

	this.timeline.addTween(cjs.Tween.get(this.sold).wait(1));

	// Слой 1
	this.instance_6 = new lib.Символ11копия();
	this.instance_6.parent = this;
	this.instance_6.setTransform(325.5,172.9,1,1,0,0,0,315,157.5);

	this.timeline.addTween(cjs.Tween.get(this.instance_6).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,885.9,346.4);


(lib.Символ40 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_1 = function() {
		this.stop();
		
		this.addEventListener("tick", fl_CustomMouseCursor.bind(this));
		
		function fl_CustomMouseCursor() {
		
		this.fon.x = - stage.mouseX / 40;
		this.fon.y = - stage.mouseY / 40;
		}
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(1).call(this.frame_1).wait(1));

	// Слой 1
	this.instance = new lib.Символ59();
	this.instance.parent = this;
	this.instance.setTransform(332.7,173.2,1,1,0,0,0,324.7,173.2);

	this.fon = new lib.Символ42();
	this.fon.parent = this;
	this.fon.setTransform(5.2,0.3,1,1,0,0,0,-2.8,0.3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance}]}).to({state:[{t:this.fon}]},1).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(8,0,885.9,346.4);


(lib.Символ1 = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_4 = function() {
		this.stop();
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(4).call(this.frame_4).wait(6));

	// Слой 5
	this.cir = new lib.sprite113();
	this.cir.parent = this;
	this.cir.setTransform(389.8,205.6,0.499,0.499,0,0,0,27.6,27.6);

	this.timeline.addTween(cjs.Tween.get(this.cir).to({x:461.8},4).to({x:389.8},5).wait(1));

	// Слой 4
	this.instance = new lib.Анимация3("synched",0);
	this.instance.parent = this;
	this.instance.setTransform(-13.9,83.8);

	this.instance_1 = new lib.Анимация4("synched",0);
	this.instance_1.parent = this;
	this.instance_1.setTransform(-13.9,83.8);
	this.instance_1.alpha = 0;
	this.instance_1._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).to({_off:true,alpha:0},4).to({_off:false,alpha:1},5).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_1).to({_off:false},4).to({_off:true,alpha:1},5).wait(1));

	// Слой 1
	this.instance_2 = new lib.Символ12();
	this.instance_2.parent = this;
	this.instance_2.setTransform(-174.5,13,1,1,0,0,0,110.5,76);

	this.timeline.addTween(cjs.Tween.get(this.instance_2).to({x:-92.1},4).to({x:-174.5},5).wait(1));

	// for_06.png
	this.instance_3 = new lib.Символ13();
	this.instance_3.parent = this;
	this.instance_3.setTransform(-174.4,155.5,1,1,0,0,0,110,75.5);

	this.timeline.addTween(cjs.Tween.get(this.instance_3).to({x:-92},4).to({x:-174.4},5).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-285,-63,693.8,294);


// stage content:



(lib.index = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});

	// timeline functions:
	this.frame_0 = function() {
		/* Пользовательский курсор мыши
		Заменяет курсор мыши по умолчанию на указанный экземпляр символа.
		*/
		stage.canvas.style.cursor = "none";
		this.pricel.mouseEnabled = false;
		this.addEventListener("tick", fl_CustomMouseCursor.bind(this));
		
		function fl_CustomMouseCursor() {
			this.pricel.x = stage.mouseX;
			this.pricel.y = stage.mouseY;
		}
		//Чтобы восстановить курсор мыши по умолчанию, раскомментируйте следующие строки:
		//this.removeEventListener("tick", fl_CustomMouseCursor.bind(this));
		//stage.removeChild(pricel);
		//stage.canvas.style.cursor = "default";
		
		var _this = this;
		
		_this.fonbase.fon.kat.visible=false;
		_this.fonbase.fon.sold.visible=false;
		_this.pricel.visible=false;
		_this.pr1.visible=true;
		
		stage.canvas.onmouseover = fl_MouseOverHandler_5;
		
		function fl_MouseOverHandler_5()
		    {
			    _this.vibor.gotoAndPlay(1);
				_this.fonbase.gotoAndPlay(1);
				_this.pricel.visible=true;
				_this.pr1.visible=false;
				
		
		
		    }
		
		stage.canvas.onmouseout = fl_MouseOutHandler_2;
		
		
		      function fl_MouseOutHandler_2()
		    {
			    _this.vibor.gotoAndPlay(5);
				_this.pricel.visible=false;
				_this.pr1.visible=true;
				_this.fonbase.gotoAndPlay(0);
		
		
		    }
			
		this.bt1.addEventListener("mouseover", fl_ClickToGoToAndPlayFromFrame_4.bind(this));
		
		function fl_ClickToGoToAndPlayFromFrame_4()
		{
		this.pricel.gotoAndStop(2);
		_this.fonbase.fon.kat.visible=true;
		_this.fonbase.fon.sold.visible=false;
		}
		
		this.bt2.addEventListener("mouseover", fl_ClickToGoToAndPlayFromFrame_3.bind(this));
		
		function fl_ClickToGoToAndPlayFromFrame_3()
		{
		this.pricel.gotoAndStop(1);
		_this.fonbase.fon.kat.visible=false;
		_this.fonbase.fon.sold.visible=true;
		}
		document.addEventListener("click", fl_ClickToGoToWebPage);
		
		
		function fl_ClickToGoToWebPage() {
		window.open(clickTAG, "_blank");
		}
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(1));

	// Слой 3
	this.pricel = new lib.Символ10();
	this.pricel.parent = this;
	this.pricel.setTransform(-198.7,163,1,1,0,0,0,63.8,25.4);

	this.timeline.addTween(cjs.Tween.get(this.pricel).wait(1));

	// Слой 9
	this.bt2 = new lib.Символ35();
	this.bt2.parent = this;
	this.bt2.setTransform(89.6,224.3,1,1,0,0,0,90.6,62.5);
	new cjs.ButtonHelper(this.bt2, 0, 1, 2, false, new lib.Символ35(), 3);

	this.bt1 = new lib.Символ32();
	this.bt1.parent = this;
	this.bt1.setTransform(89.6,82.2,1,1,0,0,0,90.6,62.9);
	new cjs.ButtonHelper(this.bt1, 0, 1, 2, false, new lib.Символ32(), 3);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.bt1},{t:this.bt2}]}).wait(1));

	// Слой 7
	this.pr1 = new lib.Символ17();
	this.pr1.parent = this;
	this.pr1.setTransform(346.2,163,1,1,0,0,0,63.8,25.4);

	this.timeline.addTween(cjs.Tween.get(this.pr1).wait(1));

	// Слой 4
	this.instance = new lib.Символ2();
	this.instance.parent = this;
	this.instance.setTransform(475,-26.4,1,1,0,0,0,213,77.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// Слой 2
	this.vibor = new lib.Символ1();
	this.vibor.parent = this;
	this.vibor.setTransform(292.5,143,1,1,0,0,0,110.5,76);

	this.timeline.addTween(cjs.Tween.get(this.vibor).wait(1));

	// Слой 1
	this.fonbase = new lib.Символ40();
	this.fonbase.parent = this;
	this.fonbase.setTransform(299.2,150.8,1,1,0,0,0,324.7,173.2);

	this.timeline.addTween(cjs.Tween.get(this.fonbase).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(36.3,127.6,1132.2,346.4);

})(lib = lib||{}, images = images||{}, createjs = createjs||{}, ss = ss||{});
var lib, images, createjs, ss;